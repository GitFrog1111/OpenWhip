...
function createUnicodeInputEvents(message) {
  const events = [];
  for (let index = 0; index < message.length; index += 1) {
    const wScan = message.charCodeAt(index);
    const keyboardInput = {
      wVk: 0,
      wScan,
    ...
    events.push(
      ...
      { type: INPUT_KEYBOARD, ki: { ...keyboardInput, dwFlags: KEYEVENTF_UNICODE | KEYUP } },
    );
  }
  return events;
}
...
function createWindowsDriver(deps) {
  const { keybdEvent, sendInput, inputSize, sleep = defaultSleep } = deps;
  if (typeof keybdEvent !== 'function') {
    throw new Error('Windows input requires keybdEvent');
  }
  if (typeof sendInput !== 'function') {
    throw new Error('Windows input requires sendInput');
  }
  if (!Number.isInteger(inputSize) || inputSize <= 0) {
    throw new Error('Windows input requires a positive integer inputSize');
  }
    ...
    async execute(steps, message) {
      for (const step of steps) {
        if (step.type === 'keystroke') {
          ...
          const events = createUnicodeInputEvents(message);
          const inserted = sendInput(events.length, events, inputSize);
          if (inserted !== events.length) {
            throw new Error(`SendInput inserted ${inserted} events; requested ${events.length}`);
          }
